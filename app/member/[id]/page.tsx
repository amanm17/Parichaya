...
export default async function MemberPage({ params }: { params: { id: string } }) {
  ...
  const connected = relationships.filter((edge) => edge.from_person === person.id || edge.to_person === person.id);
      ...
      <section className="card p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-900">{person.name}</h1>
            <p className="mt-1 text-sm text-slate-500">{person.nickname || 'No nickname added yet'}</p>
            <p className="mt-5 max-w-2xl text-sm leading-7 text-slate-600">{person.description || 'This profile can be enriched with more details, photos, and story snippets.'}</p>
          </div>
          <div className="grid gap-3 rounded-3xl bg-slate-50 p-5 text-sm text-slate-600 sm:grid-cols-2 lg:min-w-[320px]">
            <div>
              ...
              <p className="mt-1 font-medium text-slate-900">{person.gender ?? 'unspecified'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 font-medium text-slate-900">{person.is_deceased ? 'Deceased' : 'Living'}</p>
            </div>
            <div>
      ...
      <section className="grid gap-5 lg:grid-cols-[1fr_0.95fr]">
        <div className="card p-6">
          ...
          <div className="mt-4 space-y-3">
            {connected.map((edge) => {
              const otherId = edge.from_person === person.id ? edge.to_person : edge.from_person;
              const other = peopleById.get(otherId);
              return (
                <div key={edge.id} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                  <span className="font-medium text-slate-900">{edge.relation_type.replace('_', ' ')}</span>
                  {' '}with{' '}
                  {other ? <Link href={`/member/${other.id}`} className="font-medium text-root">{other.name}</Link> : 'Unknown member'}
                </div>
              );
            })}
          </div>
        ...
        <div className="card p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <Link href="/tree" className="btn-primary">Open in tree</Link>
            <Link href="/updates" className="btn-secondary">View updates</Link>
            <Link href="/add" className="btn-secondary">Add related member</Link>
          </div>
          <p className="mt-4 text-sm leading-6 text-slate-500">
            Later versions can add profile galleries, event memories, edit history, and multiple contextual kinship labels.
          </p>
        </div>
      </section>
    </main>
